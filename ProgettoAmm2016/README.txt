...
Il progetto è stato principalmente realizzato per il browser Google Chrome per via della maggior compatibilità di proprietà css rispetto aglia altri browser.
...
Inoltre, è stata implementata una funzionalità Ajax nella pagina filtrocliente.js per poter filtrare la lista degli articoli tramite input.
...
Clienti e Venditori sono tipologie di utente rappresentate da una classe a sè stante, ciò vuol dire che non hanno alcun collegamento tra loro (nessuna superclasse 'Utente').

Nel database sono già presenti svariati articoli, ognuno dei quali è stato 'messo in vendita' da un account di un venditore.